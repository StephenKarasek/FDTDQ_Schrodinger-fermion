function GRAPH_MomentumResponse_PopulationLevelNorm_VIEW(TEXT_, SIM, PULSE, taxis, xaxis, faxis, dF, ndF)
%% Spectral Analysis
close all hidden;
CONSTANTS;
...
% load([TEXT_.SaveDirectory '/PopulationStateNorm_VIEW'], 'PopulationStateNorm_VIEW');
load([TEXT_.SaveDirectory '/PopulationStateNorm_VIEW']);
...
switch Estates_
    case 1
        Eplt = plot(taxis, PopulationStateNorm_VIEW(:,1));
...
%             set(Eplt(1), 'linewidth',1.5, 'marker', 'o', 'markerfacecolor', [0 0 0.7]) ;       
    ...
    case 2
        Eplt = plot(taxis, PopulationStateNorm_VIEW(:,1),...
            taxis, PopulationStateNorm_VIEW(:,2));
        hleg = legend('E_0', 'E_1');
...
%         set(Eplt(1), 'linewidth',1.5, 'marker', 'o', 'markerfacecolor', [0 0.7 0]) ;       
    ...
    case 3
        Eplt = plot(taxis, PopulationStateNorm_VIEW(:,1),...
            taxis, PopulationStateNorm_VIEW(:,2),...
            taxis, PopulationStateNorm_VIEW(:,3));
...
%         set(Eplt(4), 'color', [0 0 0.7]);
    ...
    case 4
        Eplt = plot(taxis, PopulationStateNorm_VIEW(:,1),...
            taxis, PopulationStateNorm_VIEW(:,2),...
            taxis, PopulationStateNorm_VIEW(:,3),...
            taxis, PopulationStateNorm_VIEW(:,4));
...
%         set(Eplt(5), 'color', [0 0 0.7]);
    ...
    case 5
        Eplt = plot(taxis, PopulationStateNorm_VIEW(:,1),...
            ...
            taxis, PopulationStateNorm_VIEW(:,3),...
            taxis, PopulationStateNorm_VIEW(:,4));%,...
%             taxis, PopulationStateNorm_VIEW(:,5));
...
%%
saveas(SpectralPopStatefig, [TEXT_.saveVisual '\{FFT_PopLvl_view}.' TEXT_.SaveGraphicName]);
